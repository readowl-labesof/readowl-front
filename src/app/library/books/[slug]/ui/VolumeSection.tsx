...
type Props = {
  ...
  chapterViewsBySlug?: Record<string, number>; // optional batch views map
};

export default function VolumeSection({ title, volumeId, chapters, canManage = false, onRename, onDelete, onEditChapter, onDeleteChapter, slug, chapterViewsBySlug }: Props) {
    ...
    <div className="border-2 border-readowl-purple rounded mb-3 bg-readowl-purple-light">
      ...
      <div className={`overflow-hidden transition-[max-height] duration-300 ${open ? 'max-h-[2000px]' : 'max-h-0'}`}>
        <div className="p-3 space-y-2">
          ...
          {first.map((c) => {
            const s = c.title ? slugify(c.title) : '';
            const v = s && chapterViewsBySlug ? chapterViewsBySlug[s] ?? null : null;
            return (
              <ChapterCard key={c.id} slug={slug} chapter={c} canManage={canManage} onEditChapter={onEditChapter} onDeleteChapter={onDeleteChapter} initialViews={v} />
            );
          })}
