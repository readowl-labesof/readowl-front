...
export default function AdminCard({ user }: AdminCardProps) {
  ...
  const handleClick = () => { router.push(`/admin/users/${user.id}`); };
  return (
    <div onClick={handleClick} className="bg-purple-600 rounded-2xl p-5 flex items-center gap-4 text-white shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-[1.02] cursor-pointer hover:bg-purple-700">
      ...
      <div className="flex-1">
        <div className="flex items-center gap-3">
          ...
          <span className="font-semibold text-lg text-white">{user.name || user.email}</span>
          <span className="bg-purple-800/90 text-white text-sm px-3 py-1.5 rounded-full flex items-center gap-2 ml-auto shadow-lg font-medium"><Shield size={14} />Admin</span>
        </div>
        {user.description && (<p className="text-white/80 text-sm mt-1 line-clamp-2">{user.description}</p>)}
      ...
      <div className="text-white/60"><span className="text-sm">→</span></div>
    </div>
  );
}
