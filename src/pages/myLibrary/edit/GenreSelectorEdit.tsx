...
export const GenreSelectorEdit: React.FC<GenreSelectorEditProps> = ({
  filteredGenres,
  genreFilter,
  onFilter,
  selectedGenres,
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2">
          {filteredGenres.map((g) => {
            ...
            return (
              <label
                key={g}
                className={`flex items-center justify-center px-2 py-1 cursor-pointer text-[11px] font-medium select-none transition border border-white/10 whitespace-pre-line text-center leading-tight min-h-[34px] ${
                  checked
                    ? "bg-readowl-purple-extradark text-white ring-2 ring-white/30"
                    : "bg-readowl-purple-light text-white/90 hover:bg-readowl-purple-dark"
                }`}
              >
                <input
                  type="checkbox"
