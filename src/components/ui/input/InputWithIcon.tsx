...
const InputWithIcon: React.FC<InputWithIconProps> = ({ icon, className = "", error, rightIcon, hideErrorText, ...props }) => (
  <div className="relative mb-4">
    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-readowl-purple-medium">{icon}</span>
    <input
    ...
    {rightIcon && (
      <span className="absolute right-3 top-1/2 -translate-y-1/2 cursor-pointer select-none text-readowl-purple-dark">{rightIcon}</span>
    )}
  </div>
);
