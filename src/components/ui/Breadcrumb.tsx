...
export function Breadcrumb({ items, className = '', anchor = 'static', showHome = false, homeHref = '/home', tone = 'dark' }: BreadcrumbProps) {
  ...
  const basePos = anchor === 'top-left' ? 'absolute top-2 left-3' : anchor === 'top-center' ? 'absolute top-2 left-1/2 -translate-x-1/2' : ''
  ...
  return (
    <nav aria-label="Breadcrumb" className={`${basePos} ${className}`}>
      <ol className={`m-0 p-0 flex flex-wrap items-center break-words leading-tight text-sm ${textBase}`}>
        {list.map((item, idx) => {
          const isLast = idx === list.length - 1
          return (
            <li key={`${item.label}-${idx}`} className="flex items-center">
              {item.href && !isLast ? (
                <Link to={item.href} className={`break-words ${linkHover} transition-colors underline-offset-2 hover:underline`}>
                  {item.label}
                </Link>
              ) : (
...
// Automatic breadcrumb based on current location path; segments prettified.
export function BreadcrumbAuto({ base = '/home', labelMap = {}, className = '', anchor = 'static', tone = 'dark' }: { base?: string; labelMap?: Record<string, string>; className?: string; anchor?: Anchor; tone?: Tone }) {
  const { pathname } = useLocation()
  const path = pathname || base
  const isErrorContext = (() => {
    if (!path) return false
    const direct = path === '/403' || path === '/erro' || path === '/_not-found'
