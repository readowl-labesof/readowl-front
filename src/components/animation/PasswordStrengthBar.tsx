...
const buildTip = (
  password: string,
  score: number,
  checks: ReturnType<typeof heuristicStrength>["checks"],
) => {
...
const PasswordStrengthBar: React.FC<Props> = ({ password, tipTextColor, showPercent = true }) => {
  const { score, checks } = heuristicStrength(password);
  ...
  return (
    <div className="w-full mt-1 mb-4">
      <div
        className="relative w-full h-2 bg-gray-200 overflow-hidden group"
        role="progressbar"
