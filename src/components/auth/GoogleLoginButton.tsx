...
export function GoogleLoginButton({ className = '', label = 'Entrar com Google' }: Props) {
  ...
  return (
    <button
      type="button"
      onClick={handleClick}
      className={`w-full flex items-center justify-center gap-2 bg-white text-readowl-purple font-semibold py-2 px-4 border border-gray-300 rounded-md hover:bg-gray-100 transition mb-3 ${className}`}
      aria-label={label}
    >
